...
- **Map Obstacles, Items, and Doors:** `.admap` v4 stores obstacle rectangles with type, sprite ID, size, and timing data. `.admap` v5 adds item placements. `.admap` v6 adds door trigger placements with tile position/size, lock mode, required item, target screen/tile, sprite ID, and opening animation fields. Item placements can use legacy weapon/ammo/health pickup behavior or reference a project-wide item definition. Multiple obstacle types and multiple instances may be placed per screen. Obstacles, items, and doors share one `.admap` per screen but are authored in separate editor panels; every panel that writes the file re-reads and preserves the slices it does not own, so saving one category never wipes the others. Older `.admap` versions remain loadable.
