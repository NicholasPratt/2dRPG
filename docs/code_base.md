...
- New/reset/metadata-loaded `SpriteDocument` instances initialize `palette` with transparent followed by the complete shared `atari2600::kNtscPalette` array (129 entries total). The tool rail renders an 8-column grid whose swatch size is derived from the available rail width. Transparent is shown with a checkerboard/cross marker and is also available in the replace/add color selectors; primary and secondary selection behavior is unchanged.
