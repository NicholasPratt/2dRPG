...
- Walking SFX: put `.ogg`/`.wav` files in `assets/game/sfx/walking`, choose one from Screens -> Edit Music/SFX, and confirm it loops only while the player is moving on that screen and stops in menus/dialogue/transition.
- Music exclusivity: move repeatedly between screens with different music, the same music, and no music. Confirm the previous track stops before a replacement starts, the same track is not doubled/restarted unnecessarily, silence is respected, and door SFX plus walking SFX still mix over the single active track.
...
- Linked door pairing: create a door, set its Target screen, set Target Tile X/Y with the point-and-click spawn picker, and Apply. Confirm a paired door is auto-created at the centre of the target screen, both doors reference each other (targetDoorId), reopening either screen shows the linked door, and runtime arrival uses the configured target tile. Confirm v10 maps load and upgrade to v11 on save.
...
- Project folders: create a project and confirm the declared `assets/raw`, `assets/game`, `assets/game/sfx/doors`, `assets/game/sfx/walking`, and chapter dialogue folders exist.
- Chapter synopsis: enter multiline text for two chapters, save/reopen the project, and confirm it persists. Verify Universal variables appear at the top and only variables scoped to each chapter appear in that chapter's list.
