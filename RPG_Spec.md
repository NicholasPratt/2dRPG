...
- **Startup Flow:** On launch the editor re-opens the last project and chapter from `projects/.editor_session`; if that is missing/invalid it prompts the user with an Open Project modal. Existing projects are loaded from `projects/<project>/`; new projects create their own project folder with `assets/game/...` and `assets/raw/...` subfolders.
...
- **Frame Ordering & Resize:** Frames can be added, duplicated, deleted, and reordered (Move up / Move down) within the animation. All frames in a sprite share one canvas size; Resize rescales every frame and layer together (nearest-neighbour) and proportionally rescales the pivot and body guide.
...
- **Map Obstacles, Items, and Doors:** `.admap` v4 stores obstacle rectangles with type, sprite ID, size, and timing data. `.admap` v5 adds item placements. `.admap` v6 adds door trigger placements with tile position/size, lock mode, required item, target screen/tile, sprite ID, and opening animation fields. `.admap` v7 adds a per-obstacle `id` so each obstacle is individually listed and edited in the obstacle editor; pre-v7 obstacles load with a generated `obstacle_<n>` id. `.admap` v8 adds per-obstacle `damage` and `damageIntervalSeconds` (damage rate), so the obstacle editor edits id, type, sprite, position, size, damage, damage rate, and timing per obstacle; pre-v8 obstacles default to `damage=1` every `0.75` s. Item placements can use legacy weapon/ammo/health pickup behavior or reference a project-wide item definition. Multiple obstacle types and multiple instances may be placed per screen. Obstacles, items, and doors share one `.admap` per screen but are authored in separate editor panels; every panel that writes the file re-reads and preserves the slices it does not own, so saving one category never wipes the others. Older `.admap` versions remain loadable.
...
- `.adgame` v13 project-level data including state variable definitions, reusable effect definitions, enemy type definitions with per-type attack definitions, NPC type definitions, weapon definitions, item definitions, shop inventory, projectile wall behavior, and font path.
...
- **Door system:** per-screen door placements are stored in `.admap` v6 and edited from `Edit Doors`. Runtime doors show an interaction prompt when the player stands on or near the trigger rectangle. Free-use doors transition to their target screen/tile; locked doors and missing-key doors show feedback; required-item doors can optionally consume one matching inventory item.
...
- **HUD and inventory additions:** melee-weapon indicator rendered above the health hearts. Pressing `I`, controller north face button, Select, or Start toggles an inventory overlay with a dedicated money header and an AMMO section. Inventory rows render item sprites as pictograms and show stack/value counts as pictogram/number pairs. While open, Up/Down, W/S, D-pad, or left stick selects an item and E/Space/Enter/controller south face button uses usable item types.
...
- Door opening animation playback from the stored opening animation field.
